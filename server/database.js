...
const getUserMessages = function(db, userID) {
  return db.query(`
  SELECT DISTINCT products.owner_id, products.id AS product_id, messages.id AS message_id, messages.from_user_id, messages.to_user_id, products.name, products.photo_url, message_text, u3.name AS to_name, u2.name AS from_name
  FROM messages
